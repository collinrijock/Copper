...
| C. MCP integration (server, then client) | Part 1b | **Felipe's**, not Collin's — see note at top. Server **shipped** (`Fork/MCP/`, Settings › Agents, Playwright-MCP tool names, `--mcp-stdio` bridge, `docs/agents.md`). **Jev mode shipped** (`Fork/MCP/Ultrafast.swift`: jev-ultrafast in the open tab — `jev_run` / `jev_step` / `jev_observe` / `jev_extract`, Copy-prompt pills; 10-game Wikipedia bench 5.3× median vs Sonnet on the Playwright tools). **CLI shipped** (`copper` shim + `Copper --cli`, with `copper run` / observe / extract / screenshot commands; `copper setup phi|claude|cli|status`). CLI launch is now opt-in (`--launch` / `COPPER_LAUNCH=1`) with session list/restore and guarded backups. **Terminal setup shipped** (`Fork/MCP/Setup.swift`: token-free phi/Claude/CLI setup and status). **Client (C2) shipped** as a pane beside the page (⌘E, `Fork/Agent/`): chat with Copper's tools bound locally + mcp.json servers (HTTP + stdio). Ask on page (#16) = ⌘⇧E. **grunts link shipped** (`Fork/MCP/Link.swift` + `LinkWire.swift`: Copper dials out to grunts FluxBots and serves its MCP tools to the owner's granted grunts bots; Settings › Agents › grunts card, `copper link …`). **Headless mode** (`Fork/Headless.swift`, `docs/headless.md`: `Copper --headless` / `SEARCH_HEADLESS=1` — accessory app, window parked off every display so WebKit still paints, dialogs auto-declined, no activation; `copper intelligence status\|set\|reload` + SIGHUP reload of `intelligence.json`; `SEARCH_MCP_PORT` override; `headless` in `copper health --json`) for the grunts Mac mini's `dev.grunts.copper` LaunchAgent. Moves into the widget board (D7) when D exists |
