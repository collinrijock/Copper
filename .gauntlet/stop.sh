...
if [ "$WORLD" = all ]; then
    ...
    pkill -f "Copper-gauntlet/.*/build/Copper.app/Contents/MacOS/Copper" 2>/dev/null || true
    pkill -f "Developer/Copper/build/Copper.app/Contents/MacOS/Copper" 2>/dev/null || true
    exit 0
fi
...
for pid in $(pgrep -f "build/Copper.app/Contents/MacOS/Copper"); do
    if ps eww -p "$pid" 2>/dev/null | grep -q "SEARCH_PROBE=$WORLD\b"; then kill "$pid" 2>/dev/null || true; fi
done
sleep 0.3
